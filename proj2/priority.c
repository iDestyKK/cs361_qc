/*
 * COSC 361 - Project 2D: Priority Based Scheduling
 *
 * Description:
 *     Simulates the Priority Based scheduling algorithm.
 ...
 *
 *     gcc --std=gnu89 -o priority priority.c lib/cn_vec/cn_vec.c .......
 *
 * Author:
 *     Clara Van Nguyen
 */
...
int cmp_jobs(const void* a, const void* b) {
	PRIORITY_PROCESS *ja = a,
	                 *jb = b;
	//Check if the priority values are the same or not
	if (ja->priority == jb->priority) {
		//If so, check if arrival times are the same...
		...
		if (ja->priority < jb->priority) return -1;
		if (ja->priority > jb->priority) return  1;
	}
}
...
main(int argc, char** argv) {
	...
	if (argc < 2) {
		...
		fprintf(stderr, "Usage: priority job_file [-i IMG_PATH] [-x] [-s] [-n] [-d]\n");
		return -1;
	}
	...
	CN_VEC jobs = cn_vec_init(PRIORITY_PROCESS);
	...
	for (; nc < cn_vec_size(jobs); nc++) {
		PRIORITY_PROCESS* proc = cn_vec_at(jobs, nc);
		job_names[nc] = itos(proc->job_id);
	}

	CN_VEC process_queue     = cn_vec_init(PRIORITY_PROCESS);
	...
	while (CN_TRUE) {
		...
		for (; i < cn_vec_size(jobs); i++) {
			...
			if (process->arrival_time == time) {
				PRIORITY_PROCESS* temp = (PRIORITY_PROCESS *) malloc(sizeof(PRIORITY_PROCESS));
				memcpy(temp, process, sizeof(PRIORITY_PROCESS));
		...
		PRIORITY_PROCESS* vlist;
		if (cn_vec_size(process_queue) > 0) {
			if (time == 0 || preemptive == CN_TRUE) {
				vlist = cn_vec_data(process_queue);
				qsort(vlist, cn_vec_size(process_queue), sizeof(PRIORITY_PROCESS), cmp_jobs);
			} else {
				vlist = cn_vec_data(process_queue);
				vlist++;
				qsort(vlist, cn_vec_size(process_queue) - 1, sizeof(PRIORITY_PROCESS), cmp_jobs);
			}
		}
		...
		if (!cn_vec_empty(process_queue)) {
			//Get top element
			PRIORITY_PROCESS* cur_proc = cn_vec_at(process_queue, 0),
			                * new_proc;
			if (simulate == CN_TRUE)
			...
			for (; ii < cn_vec_size(process_queue); ii++) {
				PRIORITY_PROCESS* iterate_process = cn_vec_at(process_queue, ii);
			...
			if (cur_proc->cpu_burst <= 0) {
				...
				PRIORITY_PROCESS* vlist = cn_vec_data(process_queue);
				qsort(vlist, cn_vec_size(process_queue), sizeof(PRIORITY_PROCESS), cmp_jobs);
			}
		}
